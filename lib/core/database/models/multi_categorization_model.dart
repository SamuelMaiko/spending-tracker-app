...
class MultiCategorizationModel {
  ...
  Future<MultiCategorizationList?> getListById(int id) async {
    return await (_database.select(_database.multiCategorizationLists)
  ...
  Future<List<MultiCategorizationList>> getListsByTransactionId(int transactionId) async {
    return await (_database.select(_database.multiCategorizationLists)
          ..where((list) => list.transactionId.equals(transactionId)))
        .get();
  }
  ...
  Future<List<MultiCategorizationList>> getUnappliedLists() async {
    return await (_database.select(_database.multiCategorizationLists)
          ..where((list) => list.isApplied.equals(false)))
        .get();
  }
  ...
  Future<int> createList({
    ...
    return await _database.into(_database.multiCategorizationLists).insert(
  ...
  Future<bool> updateList(MultiCategorizationList list) async {
    return await _database.update(_database.multiCategorizationLists).replace(list);
  }

  /// Delete a list
  Future<int> deleteList(int id) async {
    // First delete all items in the list
    await _database.delete(_database.multiCategorizationItems)
      ..where((item) => item.listId.equals(id));
    
    // Then delete the list
    return await (_database.delete(_database.multiCategorizationLists)
          ..where((list) => list.id.equals(id)))
        .go();
  }
  ...
  Future<bool> markListAsApplied(int listId) async {
    return await (_database.update(_database.multiCategorizationLists)
          ..where((list) => list.id.equals(listId)))
        .write(MultiCategorizationListsCompanion(
  ...
  Future<List<MultiCategorizationItem>> getListItems(int listId) async {
    return await (_database.select(_database.multiCategorizationItems)
  ...
  Future<int> addItemToList({
    ...
    return await _database.into(_database.multiCategorizationItems).insert(
  ...
  Future<bool> updateListItem(MultiCategorizationItem item) async {
    return await _database.update(_database.multiCategorizationItems).replace(item);
  }

  /// Delete list item
  Future<int> deleteListItem(int itemId) async {
    return await (_database.delete(_database.multiCategorizationItems)
  ...
  Future<bool> canApplyList(int listId) async {
    ...
    return (totalAmount - transaction.amount).abs() < 0.01; // Allow small floating point differences
  }
}
